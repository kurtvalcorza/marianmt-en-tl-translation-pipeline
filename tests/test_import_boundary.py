...
from marianmt_translation_pipeline.pipeline import (
    MANIFEST_NAME,
    MODEL_ID,
    MODEL_REVISION,
...
_CONFIG = json.dumps({"architectures": ["MarianMTModel"], "model_type": "marian"}).encode()
...
def test_from_pretrained_refuses_without_snapshot_before_model_imports(tmp_path, forbid_model_imports):
    with pytest.raises(FileNotFoundError, match="allow_download=False"):
        MarianMTTranslationPipeline.from_pretrained(device="cpu", weights_dir=tmp_path, allow_download=False)
